...
class FormSpec extends ObjectBehavior
{
    ...
    {
        $this->close()->toHtml()->shouldBe("</form>");
    }
    ...
    function it_can_be_given_a_model()
    {
        ...
        foreach ($types as $type => $expected) {
            $this->$type(new Foo())->toHtml()->shouldBe($expected);
        }
    }
    ...
    function it_can_show_input()
    {
        ...
        foreach ($types as $type => $expected) {
            $this->$type('foo', 'bar', ['class' => 'baz'])->toHtml()->shouldBe(
                '<input class="form-control baz" name="foo" type="' . $expected . '" value="bar">'
            );
            $this->$type('foo', 'bar')->toHtml()->shouldBe(
    ...
    {
        $this->password('foo', ['class' => 'baz'])->toHtml()->shouldBe(
    ...
    {
        $this->label('foo')->toHtml()->shouldBe(
            '<label for="foo" class="control-label">Foo</label>'
        );
        $this->label('foo', 'bar')->toHtml()->shouldBe(
